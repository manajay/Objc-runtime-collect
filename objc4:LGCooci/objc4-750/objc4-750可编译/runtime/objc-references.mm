...
void _object_remove_assocations(id object) {
    /// 先创建一个临时的 vector 数组, 后面会将需要释放的 关联对象都放在这里面
    vector< ObjcAssociation,ObjcAllocator<ObjcAssociation> > elements;
    {
        ///获取manager
        AssociationsManager manager;
        /// 获取全局的 manager 的hashmap
        AssociationsHashMap &associations(manager.associations());
        if (associations.size() == 0) return;
        /// 获取 包装的 对象
        disguised_ptr_t disguised_object = DISGUISE(object);
        /// 根据包装的对象 查找其对应的 关联对象 hashmap
        AssociationsHashMap::iterator i = associations.find(disguised_object);
        if (i != associations.end()) {
            // copy all of the associations that need to be removed.
            // i-> first 为 disguised_object , 其 second 就是对象的 ObjectAssociationMap
            ObjectAssociationMap *refs = i->second;
            /// 迭代获取 map中的所有kv 键值对, second 就是关联对象 ObjcAssociation 类, first 是我们定义的常量
    ...
    // the calls to releaseValue() happen outside of the lock. 遍历关联对象数组, 进行对应 内存管理语义的销毁策略
