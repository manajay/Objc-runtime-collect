...
// SEL 并不直接是 c字符串, 还需要运行时映射到字符串才可以
...
// The order of these bits is important.  查看 这个图片 https://raw.githubusercontent.com/summertian4/Images/master/blog/blog_Advanced-iOS-Study-objc-Memory-2-01.png
#define SIDE_TABLE_WEAKLY_REFERENCED (1UL<<0) // 是否有弱引用指针
#define SIDE_TABLE_DEALLOCATING      (1UL<<1)  // MSB-ward of weak bit // 是否在析构
#define SIDE_TABLE_RC_ONE            (1UL<<2)  // MSB-ward of deallocating bit  表示值所在内存的第3位开始存放引用计数数值
#define SIDE_TABLE_RC_PINNED         (1UL<<(WORD_BITS-1)) // 是否溢出

#define SIDE_TABLE_RC_SHIFT 2
#define SIDE_TABLE_FLAG_MASK (SIDE_TABLE_RC_ONE-1)

// RefcountMap disguises(假装隐瞒) its pointers because we
...
/// 一个对象 -> 一个SideTable , 但是一个SideTable 包含多个 对象 原因看 SideTables
struct SideTable {
...
// Do it the hard way.
// 这里说不能使用 c++的构造函数, 因为 libc调用时机比 构造函数要早, 另外也不能用全局的指针, 因为
alignas(StripedMap<SideTable>) static uint8_t 
    SideTableBuf[sizeof(StripedMap<SideTable>)];

// StripedMap 是什么呢, 模板类, 能存储 spinlock_t 或者包含 spinlock_t 的结构体的 map

static void SideTableInit() {
    // SideTableBuf[4095]: <nil>
    new (SideTableBuf) StripedMap<SideTable>();
}

/// 特别关键的 一个全局 散列表 内部处理了引用技术和 弱引用对象管理表.
// 因为 StripedMap 的缘故, 取 sidetable的时候是a线程安全的
static StripedMap<SideTable>& SideTables() {
    // 摘自desgard.com:  reinterpret_cast 为 C++ 标准转换运算符 作用是 处理无关类型之间的转换。该关键字会产生一个新值，并保证与原参数（expression）拥有完全相同的比特位。
...
// Update a weak variable. 更新弱指针
// If HaveOld is true, the variable has an existing value 
//   that needs to be cleaned up. This value might be nil. HaveOld为 真, 旧值需要被清理,旧值可能为nil
// If HaveNew is true, there is a new value that needs to be  HaveNew 为真, 新值需要被赋值给这个弱指针, 新值可能为nil
//   assigned into the variable. This value might be nil.

// If CrashIfDeallocating is true, the process is halted if newObj is  CrashIfDeallocating 为真, 当新值析构时或者新值不支持弱引用时, 该过程要被终止, 为假 则用 nil更新弱指针
...
storeWeak(id *location, objc_object *newObj)
{
    ...
    /// 防止 类初始化 调用 storeWeak  死锁的作用
    Class previouslyInitializedClass = nil;
    id oldObj;
    SideTable *oldTable;
    SideTable *newTable;

    // SideTables 内部是基于自选锁取值的, 所以
 ...
 retry:
    if (haveOld) {
        oldObj = *location;
        // 获取旧对象的 sidetable, 多个对象可能有一个 SideTable
    ...
    // 加锁 对两个sidetable 加锁, 小的先加锁
    SideTable::lockTwo<haveOld, haveNew>(oldTable, newTable);
    
    if (haveOld  &&  *location != oldObj) { // 这里说的什么意思, 并发发现数据有问题吗,比如其他线程进行了修改等, 然后需要解锁重新获取
        SideTable::unlockTwo<haveOld, haveNew>(oldTable, newTable);
        goto retry;
    }

    // Prevent a deadlock between the weak reference machinery通过保证弱引用对象的isa已经被初始化, 来 防止弱引用和 initialize形成死锁的出现
    ...
    if (haveNew  &&  newObj) {
        ...
        // 保证 当前类不是之前的 进行 isInitialized 的类, 因为这里有retry递归调用
        if (cls != previouslyInitializedClass  &&  
            !((objc_class *)cls)->isInitialized()) 
        {
            // 如果要 initialize 的话 就要 解锁新旧 sidetable
            SideTable::unlockTwo<haveOld, haveNew>(oldTable, newTable);
            // 发送 `+initialize`  初始化消息给当前的类 (如果父类没有初始化则初始化父类, 递归进行)
            _class_initialize(_class_getNonMetaClass(cls, (id)newObj));

            // If this class is finished with +initialize then we're good.
            // If this class is still running +initialize on this thread 
            // (i.e. +initialize called storeWeak on an instance of itself) 类实例 的 `+initialize` 对调用 `storeWeak` 方法可能导致导致死锁
            // then we may proceed but it will appear initializing and 
            // not yet initialized to the check above. // 所以要设置 previouslyInitializedClass 记住上一个初始化的类, 这样就不会重复进去这个方法了, 不过这里 我不明白的是 previouslyInitializedClass 又不是全局变量,
            // Instead set previouslyInitializedClass to recognize it on retry.
            // initialize 具体的实现应该时汇编处理的 暂时没有找到
    ...
    if (haveOld) {
        // 清除当前weak_table 中记录的 旧值
        weak_unregister_no_lock(&oldTable->weak_table, oldObj, location);
    }

    // Assign new value, if any.
    if (haveNew) {
        // 之所以返回这个值, 是为了 不需要或者存储不了的时候
        newObj = (objc_object *)
            weak_register_no_lock(&newTable->weak_table, (id)newObj, location, 
                                  crashIfDeallocating);
        // weak_register_no_lock returns nil if weak store should be rejected
        // 如果为空的话, 返回的location的值就是一个nil
        // Set is-weakly-referenced bit in refcount table.
        if (newObj  &&  !newObj->isTaggedPointer()) {
            // 这里是在isa_t中进行记录, 方便对象的相关内存操作
 ...
 * 
 * @param location The address of the weak pointer itself  注意这里是弱指针的地址不是值
 * @param newObj The new object this weak ptr should now point to 对象的堆区地址
 * 
 * @return \e newObj
 */
id
 ...
 * 并发修改时 该函数不是线程安全的
 * @param location Address of __weak ptr.   取地址符号, 也就是指针的地址, 不是指针的值
...
objc_object::sidetable_getExtraRC_nolock()
{
    assert(isa.nonpointer);
    SideTable& table = SideTables()[this];
    // 获取引用计数map, 根据对象地址 取出来 map中的迭代器元素
    RefcountMap::iterator it = table.refcnts.find(this);
    if (it == table.refcnts.end()) return 0;
    // 取出来值 还得 右移 获取真正的引用计数值, 是因为 it->second 存储了多个值, 类似共聚体, 第一位, 第二位分别记录是否弱引用, 是否在析构
...
objc_object::sidetable_retain()
{
    ...
    table.lock();
    // 获取内部的记录值, 奇怪 这不是map的元素吗, 原来是 继承的 DenseMapBase 中的 操作符重写了, 这里取的迭代器 pair 的second 也就是记录值
    size_t& refcntStorage = table.refcnts[this];
    if (! (refcntStorage & SIDE_TABLE_RC_PINNED)) {// 这里判断是指的 该值是不是溢出了, 下面要累计, 肯定不能溢出, 引用计数累计到溢出也是厉害了
        // 累计, 因为低两位被标记占用, 所以不能是累计1, 而已累计
...
objc_object::sidetable_tryRetain()
{
    ...
    } else if (it->second & SIDE_TABLE_DEALLOCATING) { /// 释放的时候就返回失败
        result = false;
...
objc_object::sidetable_retainCount()
{
    ...
    size_t refcnt_result = 1; // 默认是1 .再累计上引用计数map中的值
...
#if __OBJC2__
    if (fastpath(!cls->ISA()->hasCustomAWZ())) { //没有 自定义的 allocWithZone 实现
...
- (void)dealloc {
    // 调用 _objc_rootDealloc -> rootDealloc
