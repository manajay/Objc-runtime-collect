...
fixupMethodList(method_list_t *mlist, bool bundleCopy, bool sort)
{
    ...
    // fixme lock less in attachMethodLists ?
    {
        ...
        // 首先保证 SEL已经注册到 系统, 了解 SEL的注册可以看我SEL的文章 https://www.todayios.com/objective-c-sel/
    ...
    // Sort by selector address. 排序的 sorter 函数是比较的 SEL 地址, 因为查询的时候,也是根据 SEL 进行查询的
    if (sort) {
        method_t::SortBySELAddress sorter;
        std::stable_sort(mlist->begin(), mlist->end(), sorter);
    }
    
    // Mark method list as uniqued and sorted 更新标志位
    mlist->setFixedUp();
}
                   ...
                   bool baseMethods, bool methodsFromBundle)
{
    ...
    for (int i = 0; i < addedCount; i++) {
        ...
        // Fixup selectors if necessary 必要的选择子修正
        if (!mlist->isFixedUp()) {
...
attachCategories(Class cls, category_list *cats, bool flush_caches)
{
    ...
    // Count backwards through cats to get newest categories first 倒序添加分类的方法, 最后添加的方法覆盖前面的方法
    int mcount = 0;
    int propcount = 0;
    int protocount = 0;
    int i = cats->count;
...
static void addSubclass(Class supercls, Class subcls)
{
    ...
    if (supercls  &&  subcls) {
        ...
        /// 子类.nextSiblingClass -> 父类.firstSubclass
        ...
        // 标记 是否有自定义的内存管理方法 如 retain autorelease等
        if (supercls->hasCustomRR()) {
            subcls->setHasCustomRR(true);
        }

        // 标记 是否有自定义的 alloc 方法, 注意仅元类有
        if (supercls->hasCustomAWZ()) {
            subcls->setHasCustomAWZ(true);
        }

        // Special case: instancesRequireRawIsa does not propagate 
        // from root class to root metaclass 这里排除了 NSObject 的 根类
...
{
    size_t count;
    /// 实现了 load 方法的类 会在启动的时候被j调用加载, 所以启动优化会建议将不必须的代码 放在 initialize 里面
    
    // __objc_classlist 包含了 __objc_nlclslist 的信息
    if (_getObjc2NonlazyClassList(mhdr, &count)  &&  count > 0) return true;
    if (_getObjc2NonlazyCategoryList(mhdr, &count)  &&  count > 0) return true;
    return false;
}

//TODO: 运行时的准备工作 , 通过 mach-O header
...
/**********************************************************************
* addMethod
 增加方法 Method 记录, 如果旧的方法二维数组没有记录,就添加一个一维的方法列表, 以前只是以为添加的是一个方法而已
* fixme
* Locking: runtimeLock must be held by the caller
**********************************************************************/
static IMP 
addMethod(Class cls, SEL name, IMP imp, const char *types, bool replace)
{
    ...
    if ((m = getMethodNoSuper_nolock(cls, name))) {
        ...
        // 标记已经修复, 没办法 一维数组中就一个方法, 不需要修复: 一个不需要排序, SEL也已经注册了
        ...
        // 这里因为传参的限定 如 下面的 1, NO, NO, 所以只会检查 alloc 和 retain/release/autorelease/retainCount 的自定义实现
        prepareMethodLists(cls, &newlist, 1, NO, NO);
        // 增加到二维数组中 attachLists 要看具体的实现
...
_class_createInstanceFromZone(Class cls, size_t extraBytes, void *zone, 
                              ...
                              size_t *outAllocatedSize = nil) // 默认参数
{
    if (!cls) return nil;

    assert(cls->isRealized()); // 断言类已经初始化

    // Read class's info bits all at once for performance , 一次全部读取, 性能优化
    bool hasCxxCtor = cls->hasCxxCtor();// c++ 构造器
    bool hasCxxDtor = cls->hasCxxDtor(); // c++析构器
    bool fast = cls->canAllocNonpointer(); // ??? 是否能构建 共聚体 isa ??
    ...
    // 总结也就是 不论是不是指针优化, 实例的分配都是调用的 calloc 方法进行内存的分配, 大小是至少16字节
    id obj;
    if (!zone  &&  fast) {
        /// 一般走这里, 真正调用的是 calloc  , 这个方法 没有分配实际的物理内存, 仅仅是虚拟内存
        ...
        /// 这里是  raw pointer 也就是仅仅只是指针, 32位 CPU的时候是这样的,
        obj->initIsa(cls);
    }
...
id 
class_createInstance(Class cls, size_t extraBytes) // 创建对象内存
{
...
void *objc_destructInstance(id obj) 
{
    if (obj) {
        ...
        // This order is important. 顺序是 先执行 c++的 析构函数, 再移除关联对象, 这个顺序非常关键
