...
/**
 * The internal structure stored in the weak references table.  内联结构体
 * It maintains and stores  哈希表中 保存  指向对象的弱引用指针 , 这里说了超过了 REFERRERS_OUT_OF_LINE 就是set 否则是数组
...
/**
 * The global weak references table. Stores object ids as keys, key为对象,弱引用 weak_entry_t 为value
...
struct weak_table_t {
    ...
    // 存储空间
    size_t    num_entries;
    //  参与计算 对象的存储索引, hash对象的指针后,需要取模才能不超过数组
    uintptr_t mask;
    // hash key 最大偏移值
    uintptr_t max_hash_displacement;
};
