...
struct method_t {
    ...
    /// sorter 函数, 方法的排序函数 结构体里面放指针原来还可以这么处理
    struct SortBySELAddress :
        public std::binary_function<const method_t&,
                                    const method_t&, bool>
    {
        // 重载操作符, 比较SEL , 因为 SEL是结构体指针, 所以比较指针地址的大小 完全可以
...
/// 类的二进制兼容是指的什么   其中一个是 成员变量的布局,   OC2.0 增加的 non-fragile 使得即便系统升级, 基类的内存成员改变, 其依赖库也不需要重新编译, 实现是 reconcileInstanceVariables
// https://www.jianshu.com/p/3b219ab86b09
/// 同时可以学习一下 swift 的ABI 稳定指的是什么
struct ivar_t {
#if __x86_64__
...
#endif
    int32_t *offset; // 在对象中的内存偏移
    const char *name;//名称
    const char *type;//类型, 经常会 @encode 使用到
...
/**
 https://juejin.im/post/5c4598d5518825259278fb43  健壮的实例变量 (Non Fragile ivars)和脆弱的实例变量(Fragile ivars)
...
struct class_ro_t {
    uint32_t flags; // 标记
    uint32_t instanceStart; // 用于标记成员变量的分配 见 Non Fragile ivars
    uint32_t instanceSize; // 用于编辑成员变量的分配  见 Non Fragile ivars
#ifdef __LP64__
    uint32_t reserved;
#endif

    // 成员变量布局, 好像和strong 成员有关 , 与之l相似的是 weakIvarLayout
    // 运行时确实可以在动态生成的新类 添加 class_addIvar 成员, 但是不能给已经存在的类添加, 很简单的原因是 即便改了类的成员布局,但是已经生成的类实例却无法修改内存了
    const uint8_t * ivarLayout;
    
    const char * name; // 类名称
    
    /// method_list_t 一维数组,  而 rw 的是二维数组 method_array_t methods;
    method_list_t * baseMethodList; // 编译期生成的 基础方法 比如主类和extension , 不包括 category 因为 category 是运行时添加到 rw 的
    protocol_list_t * baseProtocols; // 编译期生成的 协议
    const ivar_list_t * ivars; //编译期生成的  成员列表

    const uint8_t * weakIvarLayout; //弱成员内部布局
 ...
 public:
    ...
    void attachLists(List* const * addedLists, uint32_t addedCount) {
        ...
        if (hasArray()) {
            ...
            uint32_t oldCount = array()->count; // 原有二维数组的 rows 数量
            uint32_t newCount = oldCount + addedCount; // 新增后的 rows 数量
            setArray((array_t *)realloc(array(), array_t::byteSize(newCount))); // 扩容,原因数据保留,重新分配内存大小
            array()->count = newCount; // 更新rows 记录值
            memmove(array()->lists + addedCount, array()->lists, 
                    oldCount * sizeof(array()->lists[0])); // 将原有的二维数组 向后移动 addedCount 个单位, 这样是保证后添加的方法先访问到
            memcpy(array()->lists, addedLists, 
                   addedCount * sizeof(array()->lists[0])); // 将添加的 二维数据, 添加到 原有二维数组的前头
        }
        else if (!list  &&  addedCount == 1) {
            // 0 lists -> 1 list
            list = addedLists[0]; // 没有数组的话. 直接将这个二维数组的首个元素地址(一维数组的首地址)f 赋值给 list (二维数组)
        } 
        else {
            // 1 list -> many lists
            List* oldList = list;
...
// 关于 ABI https://xiaozhuanlan.com/topic/0195376248

// 运行时生成的可读写的 rw 类信息 , 通过 realizeClass 方法实现生成 rw 然后 通过 methodizeClass来将方法属性等加载进去
struct class_rw_t {
    // Be warned that Symbolication knows the layout of this structure.
    uint32_t flags;
    uint32_t version;

    /// 只读的 类信息, 编译期生成的 , 内部的ivar_layout 编译期生成的, 所以固定无法改变, 这也是分类无法添加成员变量的原因, 分类仅仅能添加属性的set get
    const class_ro_t *ro;
    ...
    ///  addSubclass 时 更新 firstSubclass 和 nextSiblingClass ,
    ///  ①父类的firstSubclass 更新为 子类的nextSiblingClass
    ///  ② 父类的firstSubclass 更新为 当前的子类
    /// 在 flushcache -> foreach_realized_class_and_subclass_2 0->  中进行了 调用,
    Class firstSubclass;
    Class nextSiblingClass;

    // 一般是为了命名空间 会使用 mangle 的方式 给符号名称 重命名
    char *demangledName;
...
public:
    ...
    bool instancesRequireRawIsa() {
        return data()->flags & RW_REQUIRES_RAW_ISA; // 掩码
    }
    void setInstancesRequireRawIsa() {
...
struct objc_class : objc_object {
    ...
    /// https://www.desgard.com/iOS-Source-Probe/Objective-C/Runtime/weak%20%E5%BC%B1%E5%BC%95%E7%94%A8%E7%9A%84%E5%AE%9E%E7%8E%B0%E6%96%B9%E5%BC%8F.html
    /// 是否具有默认的 内存管理语言函数, ARC是没有这些的
    bool hasCustomRR() {
...
// Enumerates a class and all of its realized subclasses. :  code 我去 这是闭包啊
static inline void
